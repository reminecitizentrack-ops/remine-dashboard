        ...
        <header className="bg-white border-b border-gray-100 px-4 lg:px-6 py-3.5 flex items-center justify-between gap-4 flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={refetchAll}
              disabled={isLoading}
              className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm shadow-emerald-200"
            >
            ...
            <button
              onClick={async () => {
                // Précharger les logs de suppression pour le modal PDF
                try {
                  const res = await dashboardAPI.getDeletionLogs({ limit: 500 });
                  if (res?.success) {
                    setDeletionLogsForPDF(res.data?.logs || []);
